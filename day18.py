...
#     the next tile in this new row is a trap, ^.  The third character
#     considers .^^, which matches the second trap rule: its center and
#     right tiles are traps, but its left tile is not. Therefore, this
#     tile is also a trap, ^.  The last two characters in this new row
#     match the first and third rules, respectively, and so they are
#     both also traps, ^.
# 
...
def get_or_safe(line, i):
    """
    Return the line item or safe it i is outside
    the line
    """
...
def next_line(line):
    """
    Compute the next line from the given one
    """
    new_line = ''
    
    for i in range(len(line)):
        if get_or_safe(line, i - 1) != get_or_safe(line, i + 1):
            new_line += TRAPPED
        else:
            new_line += SAFE
...
def nb_safe(line):
    """
    Count the number of safe tiles in the given line
    """
    return line.count(SAFE)

def run(total, line):
    """
    Generate 'total' from line and return a pair (last line, number of safe tiles)
    """
    n_safe = nb_safe(line)
...
with io.open('inputs/day18.txt', 'r') as f:
    line = f.readlines()[0].strip()
    (last_line, n) = run(39, line)
    print('There are {} safe tiles'.format(n))
